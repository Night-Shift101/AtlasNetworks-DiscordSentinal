...
module.exports = {
    ...
    async execute(config, interaction) {
        const linkedRoles = require('./linkedRoles.json'); // <-- Only one JSON now
        ...
        // Find which link contains the targetRole
        ...
        // Check if the user has the permission role required
        // Check if the user has the permission role required
        if (!interaction.member.roles.cache.has(linkFound.permissionRole)) {
            console.log(`[Debug] User ${interaction.user.tag} tried to use /roleadd without required role!`); // <-- debug log
            await handleBlockedAction(
                interaction.client,
                interaction.user,
                `Attempted to assign **${targetRole.name}** to **${targetUser.username}** without the required role: <@&${linkFound.permissionRole}>`,
                interaction.guild.id
            );
            console.log(`[Debug] Called handleBlockedAction successfully.`); // <-- debug log
            
            return await interaction.reply({
                content: `❌ You do not have the required role to assign **${targetRole.name}**.\n\nRequired role: <@&${linkFound.permissionRole}>`,
                ephemeral: true
            });
        }
        ...
        const confirmationRow = new ActionRowBuilder()
            .addComponents(
                new ButtonBuilder()
                    .setCustomId('confirm')
                    .setLabel('✅ Confirm Role Assignment')
        ...
        collector.on('collect', async i => {
            ...
            if (i.customId === 'confirm') {
                await i.update({ content: `🔄 Starting role assignment for **${targetUser.username}**...`, components: [] });
